...
struct Vector4
{
	...
	Vector4(const Vector4 &rhs)
		: vec(rhs.vec), val(rhs.val)
	{
	}

	Vector4(const Vector3 &in_v, Scalar w)
		: vec(in_v), val(w)
	{
	}
	...
	Vector4 operator*(Scalar rhs)
	{
		return Vector4(vec * rhs, val * rhs);
	}
...
// Yizhou: Type agnostic Triangle using Algebra
template <typename Algebra>
...
// Yizhou: New data structures to track the indices of each vertex

// Struct to track the coordinates and normal for each vertex
// struct Vertex {
//   mpVector p;
//   mpVector norm;
// };
...
typedef std::vector<int> IndexTriangle; // Array<int, 3>
...
template <typename Algebra>
typename Algebra::Vector3 LinearInterp(const Vector4<Algebra> &p1, const Vector4<Algebra> &p2, float value)
{
	typename Algebra::Vector3 p1_v = p1.vec;
	typename Algebra::Vector3 p2_v = p2.vec;
	typename Algebra::Vector3 p;
	if (abs(Algebra::to_double(p1[3]) - Algebra::to_double(p2[3])) > 0.00001)
	{
		auto diff = p2_v - p1_v;
...
template <typename Algebra>
using FORMULA = std::function<typename Algebra::Scalar(typename Algebra::Vector3)>;
////////////////////////////////////////////////////////////////////////////////////////
...
MCShape<Algebra> MarchingCubes(int ncellsX, int ncellsY, int ncellsZ,
							   float gradFactorX, float gradFactorY, float gradFactorZ,
							   float minValue, typename Algebra::Vector3 *points);
/////////////////////////////////////////////////////////////////////////////////////////

// Yizhou: STRAIGHT implementation with point building
// MCShape MarchingCubes(float mcMinX, float mcMaxX, float mcMinY, float mcMaxY, float mcMinZ, float mcMaxZ,
//                     int ncellsX, int ncellsY, int ncellsZ, float gradFactorX, float gradFactorY, float gradFactorZ,
//                     float minValue, FORMULA formula);

template <typename Algebra>
MCShape<Algebra> MarchingCubes(float mcMinX, float mcMaxX, float mcMinY, float mcMaxY, float mcMinZ, float mcMaxZ,
							   int ncellsX, int ncellsY, int ncellsZ, float gradFactorX, float gradFactorY, float gradFactorZ,
							   float minValue, FORMULA<Algebra> formula);
...
#define CALC_GRAD_VERT_0(verts) Vector4(points[ind - YtimeZ][VAL_ID] - (verts[1])[VAL_ID], points[ind - pointsZ][VAL_ID] - (verts[4])[VAL_ID], points[ind - 1][VAL_ID] - (verts[3])[VAL_ID], (verts[0])[VAL_ID]);
#define CALC_GRAD_VERT_1(verts) Vector4((verts[0])[VAL_ID] - points[ind + 2 * YtimeZ][VAL_ID], points[ind + YtimeZ - pointsZ][VAL_ID] - (verts[5])[VAL_ID], points[ind + YtimeZ - 1][VAL_ID] - (verts[2])[VAL_ID], (verts[1])[VAL_ID]);
#define CALC_GRAD_VERT_2(verts) Vector4((verts[3])[VAL_ID] - points[ind + 2 * YtimeZ + 1][VAL_ID], points[ind + YtimeZ - ncellsZ][VAL_ID] - (verts[6])[VAL_ID], (verts[1])[VAL_ID] - points[ind + YtimeZ + 2][VAL_ID], (verts[2])[VAL_ID]);
#define CALC_GRAD_VERT_3(verts) Vector4(points[ind - YtimeZ + 1][VAL_ID] - (verts[2])[VAL_ID], points[ind - ncellsZ][VAL_ID] - (verts[7])[VAL_ID], (verts[0])[VAL_ID] - points[ind + 2][VAL_ID], (verts[3])[VAL_ID]);
#define CALC_GRAD_VERT_4(verts) Vector4(points[ind - YtimeZ + ncellsZ + 1][VAL_ID] - (verts[5])[VAL_ID], (verts[0])[VAL_ID] - points[ind + 2 * pointsZ][VAL_ID], points[ind + ncellsZ][VAL_ID] - (verts[7])[VAL_ID], (verts[4])[VAL_ID]);
#define CALC_GRAD_VERT_5(verts) Vector4((verts[4])[VAL_ID] - points[ind + 2 * YtimeZ + ncellsZ + 1][VAL_ID], (verts[1])[VAL_ID] - points[ind + YtimeZ + 2 * pointsZ][VAL_ID], points[ind + YtimeZ + ncellsZ][VAL_ID] - (verts[6])[VAL_ID], (verts[5])[VAL_ID]);
#define CALC_GRAD_VERT_6(verts) Vector4((verts[7])[VAL_ID] - points[ind + 2 * YtimeZ + ncellsZ + 2][VAL_ID], (verts[2])[VAL_ID] - points[ind + YtimeZ + 2 * ncellsZ + 3][VAL_ID], (verts[5])[VAL_ID] - points[ind + YtimeZ + ncellsZ + 3][VAL_ID], (verts[6])[VAL_ID]);
#define CALC_GRAD_VERT_7(verts) Vector4(points[ind - YtimeZ + ncellsZ + 2][VAL_ID] - (verts[6])[VAL_ID], (verts[3])[VAL_ID] - points[ind + 2 * ncellsZ + 3][VAL_ID], (verts[4])[VAL_ID] - points[ind + ncellsZ + 3][VAL_ID], (verts[7])[VAL_ID]);

// Yizhou: Helper macro to update the vertex lookup table and vertex list
...
MCShape<Algebra> MarchingCubes(int ncellsX, int ncellsY, int ncellsZ,
							   float gradFactorX, float gradFactorY, float gradFactorZ,
							   float minValue, Vector4<Algebra> *points)
{
	using Vector3 = typename Algebra::Vector3;
	...
	std::vector<IndexTriangle> triangles(3 * ncellsX * ncellsY * ncellsZ, IndexTriangle(3, 0));
	...
	Vector3
		intVerts[12];	  //linearly interpolated vertices on each edge
	...
	Vector3 factor(1.0 / (2.0 * gradFactorX), 1.0 / (2.0 * gradFactorY), 1.0 / (2.0 * gradFactorZ));
		...
		{ //y axis
			...
			for (int k = 0; k < lastZ; k++, ind++) //z axis
			{
				...
				if (edgeIndex & 1)
				{
					...
					if (i != 0 && j != 0 && k != 0)
						gradVerts[0] = CALC_GRAD_VERT_0(*verts) else gradVerts[0] = Vector4(1.0, 1.0, 1.0, 1.0);
					if (i != lastX - 1 && j != 0 && k != 0)
						gradVerts[1] = CALC_GRAD_VERT_1(*verts) else gradVerts[1] = Vector4(1.0, 1.0, 1.0, 1.0);
					indGrad |= 3;
				...
				if (edgeIndex & 2)
				{
					...
					if (i != lastX - 1 && j != 0 && k != 0)
						gradVerts[2] = CALC_GRAD_VERT_2(*verts) else gradVerts[2] = Vector4(1.0, 1.0, 1.0, 1.0);
					indGrad |= 4;
				...
				if (edgeIndex & 4)
				{
					...
					if (i != 0 && j != 0 && k != lastZ - 1)
						gradVerts[3] = CALC_GRAD_VERT_3(*verts) else gradVerts[3] = Vector4(1.0, 1.0, 1.0, 1.0);
					indGrad |= 8;
				...
				if (edgeIndex & 16)
				{
					...
					if (i != 0 && j != lastY - 1 && k != 0)
						gradVerts[4] = CALC_GRAD_VERT_4(*verts) else gradVerts[4] = Vector4(1.0, 1.0, 1.0, 1.0);

					if (i != lastX - 1 && j != lastY - 1 && k != 0)
						gradVerts[5] = CALC_GRAD_VERT_5(*verts) else gradVerts[5] = Vector4(1.0, 1.0, 1.0, 1.0);
				...
				if (edgeIndex & 32)
				{
					...
					if (i != lastX - 1 && j != lastY - 1 && k != lastZ - 1)
						gradVerts[6] = CALC_GRAD_VERT_6(*verts) else gradVerts[6] = Vector4(1.0, 1.0, 1.0, 1.0);
					indGrad |= 64;
				...
				if (edgeIndex & 64)
				{
					...
					if (i != 0 && j != lastY - 1 && k != lastZ - 1)
						gradVerts[7] = CALC_GRAD_VERT_7(*verts) else gradVerts[7] = Vector4(1.0, 1.0, 1.0, 1.0);
					indGrad |= 128;
				...
				if (edgeIndex & 1024)
				{
					...
					{
						if (i != lastX - 1 && j != 0 && k != 0)
							gradVerts[2] = CALC_GRAD_VERT_2(*verts) else gradVerts[5] = Vector4(1.0, 1.0, 1.0, 1.0);
						indGrad |= 4;
					}
					if (!(indGrad & 64))
					{
				...
				{
					int index[3] = {triTable[cubeIndex][n], triTable[cubeIndex][n + 1], triTable[cubeIndex][n + 2]};
					for (int h = 0; h < 3; h++)
							   ...
							   float minValue, FORMULA<Algebra> formula)
{
	using Vector3 = typename Algebra::Vector3;
	using Vector4 = Vector4<Algebra>;
	Vector4 *mcDataPoints = new Vector4[(ncellsX + 1) * (ncellsY + 1) * (ncellsZ + 1)];
	Vector3 stepSize((mcMaxX - mcMinX) / ncellsX, (mcMaxY - mcMinY) / ncellsY, (mcMaxZ - mcMinZ) / ncellsZ);
	...
	return MarchingCubes(ncellsX, ncellsY, ncellsZ, gradFactorX, gradFactorY, gradFactorZ, minValue, mcDataPoints);
}
