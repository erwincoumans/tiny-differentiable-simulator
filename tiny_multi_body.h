...
//    link.m_S.print("joint.S");
  ...
  void forwardDynamicsOld(const std::vector<TinyScalar>& q,
                          const std::vector<TinyScalar>& qd,
                          const std::vector<TinyScalar>& tau,
                          const TinyVector3& gravity,
                          std::vector<TinyScalar>& qdd) {
    ...
    for (int i = m_links.size() - 1; i >= 0; i--) {
      int parent = m_links[i].m_parent_index;
      // assume only revolute joints around Z axis
      m_links[i].m_U = m_links[i].m_I.mul_inv(m_links[i].m_S);
      m_links[i].m_d = m_links[i].m_S.dot(m_links[i].m_U);
      TinyScalar tau_val = get_tau_for_link(tau, i);
      m_links[i].m_u = tau_val - m_links[i].m_S.dot(m_links[i].m_pA);

      // m_links[i].m_U.print("m_U");
      // printf("m_links[%d].m_d=", i);
      // double d1 = TinyConstants::getDouble(m_links[i].m_d);
      // printf("%f\n", d);
      // printf("m_links[%d].m_u=", i);
      // double u = TinyConstants::getDouble(m_links[i].m_u);
      // printf("%f\n", u);

      if (parent >= 0) {
        TinyScalar invd = TinyConstants::one() / m_links[i].m_d;
        TinySymmetricSpatialDyad tmp =
            TinySymmetricSpatialDyad::vTimesvTranspose(m_links[i].m_U * invd,
                                                       m_links[i].m_U);
        TinySymmetricSpatialDyad Ia = m_links[i].m_IA;
        Ia -= tmp;
        // Ia.print("Ia");
        TinySpatialMotionVector tmp2 = Ia.mul_inv(m_links[i].m_c);
        // tmp2.print("tmp2");
        TinySpatialMotionVector pa =
            m_links[i].m_pA + tmp2 +
            m_links[i].m_U * (m_links[i].m_u / m_links[i].m_d);
        // pa.print("pa");
        TinySpatialMotionVector tmp3 =
            m_links[i].m_X_parent2.apply_transpose(pa);
        // tmp3.print("tmp3");
        m_links[parent].m_pA += tmp3;
        // m_links[parent].m_pA.print("pa update");
        TinySymmetricSpatialDyad i2 =
            TinySymmetricSpatialDyad::shift(Ia, m_links[i].m_X_parent2);
        m_links[parent].m_IA += i2;
        // m_links[parent].m_I.print("mI");
      }
    }

    for (int i = 0; i < m_links.size(); i++) {
      int parent = m_links[i].m_parent_index;
      TinySpatialTransform X_parent = m_links[i].m_X_parent2;
      TinySpatialMotionVector parentAccel =
          (parent >= 0) ? m_links[parent].m_a : spatial_gravity;

      TinySpatialMotionVector xpa = X_parent.apply(parentAccel);
      m_links[i].m_a = xpa + m_links[i].m_c;
      // xpa.print("xpa");
      // m_links[i].m_a.print("a'");
      // model.a[i] = X_parent.apply(model.a[parent]) + model.c[i];
      // LOG << "a'[" << i << "] = " << model.a[i].transpose() << std::endl;

      // if (model.mJoints[i].mDoFCount == 1
      {
        int qd_index = m_links[i].m_qd_index;
        TinyScalar t0 = (TinyConstants::one() / m_links[i].m_d);
        TinyScalar t1 = m_links[i].m_U.dot(m_links[i].m_a);
        TinyScalar t2 = m_links[i].m_u - t1;
        qdd[qd_index] = t0 * t2;
        m_links[i].m_a = m_links[i].m_a + m_links[i].m_S * qdd[qd_index];
        // m_links[i].m_a.print("a");
      }
    }
  }
