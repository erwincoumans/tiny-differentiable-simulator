...
void TinyWin32Window::switchFullScreen(bool fullscreen, int width, int height, int colorBitsPerPixel)
{
	LONG res;
	DEVMODE dm;
	memset(&dm, 0, sizeof(dm));
...
void TinyWin32Window::start_rendering()
{
	pumpMessage();
}
...
void TinyWin32Window::end_rendering()
{
	SwapBuffers(m_data->m_hDC);
}

float TinyWin32Window::get_time_in_seconds()
{
	return 0.f;
}
...
void TinyWin32Window::set_request_exit()
{
	m_data->m_quit = true;
}
bool TinyWin32Window::requested_exit() const
{
...
void TinyWin32Window::set_mouse_move_callback(TinyMouseMoveCallback mouseCallback)
{
	m_data->m_mouseMoveCallback = mouseCallback;
}

void TinyWin32Window::set_mouse_button_callback(TinyMouseButtonCallback mouseCallback)
{
...
void TinyWin32Window::set_keyboard_callback(TinyKeyboardCallback keyboardCallback)
{
