...
struct TinyCommonGraphicsApp
{
	...
	virtual void get_screen_pixels(unsigned char* rgbaBuffer, int bufferSizeInBytes, float* depthBuffer, int depthBufferSizeInBytes) {}
	...
	virtual void draw_text(const char* txt, int posX, int posY, float size, float colorRGBA[4]) = 0;
	virtual void draw_text_3d(const char* txt, float posX, float posY, float posZ, float size) = 0;
	virtual void draw_text_3d(const char* txt, float position[3], float orientation[4], float color[4], float size, int optionFlag) = 0;
	virtual void draw_textured_tect(float x0, float y0, float x1, float y1, float color[4], float u0, float v0, float u1, float v1, int useRGBA) = 0;
	virtual int register_cube_shape(float halfExtentsX, float halfExtentsY, float halfExtentsZ, int textureIndex = -1, float textureScaling = 1) = 0;
	virtual int register_graphics_unit_sphere_shape(EnumSphereLevelOfDetail lod, int textureId = -1) = 0;

	virtual void register_grid(int xres, int yres, const TinyVector3f& color0, const TinyVector3f& color1) = 0;
	...
	void defaultMouseMoveCallback(float x, float y)
	{
		if (m_window && m_renderer)
		{
			...
			bool isControlPressed = m_window->is_modifier_key_pressed(TINY_KEY_CONTROL);
			...
			if (isAltPressed || isControlPressed)
			{
				...
				camera->set_camera_target_position(cameraTargetPosition[0], cameraTargetPosition[1], cameraTargetPosition[2]);
			}
	...
	void defaultWheelCallback(float deltax, float deltay)
	{
		if (m_renderer)
		{
			TinyVector3f cameraTargetPosition, cameraPosition, cameraUp = TinyVector3f(0, 0, 0);
