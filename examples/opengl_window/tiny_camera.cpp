...
#endif
static void b3CreateLookAt(const TinyVector3f& eye, const TinyVector3f& center, const TinyVector3f& up, float result[16])
{
...
int TinyCamera::get_camera_up_axis() const
{
...
void TinyCamera::update()
{
	...
	float pitchRad = m_data->m_pitch * float(0.01745329251994329547);  // rads per deg
	float rollRad = 0.0;
...
void TinyCamera::get_camera_projection_matrix(float projectionMatrix[16]) const
{
...
float TinyCamera::get_camera_yaw() const
{
	return m_data->m_yaw;
}
...
float TinyCamera::get_camera_pitch() const
{
	return m_data->m_pitch;
}
float TinyCamera::get_aspect_ratio() const
{
