...
///We add the do/while so that the statement "if (condition) TinyPrintf("test"); else {...}" would fail
...
#define TinyPrintf(...) TinyOutputPrintfVarArgsInternal(__VA_ARGS__)
	//#define TinyPrintf(...) do {TinyOutputPrintfVarArgsInternal("TinyPrintf[%s,%d]:",__FILE__,__LINE__);TinyOutputPrintfVarArgsInternal(__VA_ARGS__); } while(0)
	//#define TinyPrintf TinyOutputPrintfVarArgsInternal
	//#define TinyPrintf(...) printf(__VA_ARGS__)
	//#define TinyPrintf(...)
#define TinyWarning(...) do{	TinyOutputWarningMessageVarArgsInternal("TinyWarning[%s,%d]:\n", __FILE__, __LINE__);TinyOutputWarningMessageVarArgsInternal(__VA_ARGS__);} while (0)
#define TinyError(...)do	{TinyOutputErrorMessageVarArgsInternal("TinyError[%s,%d]:\n", __FILE__, __LINE__);TinyOutputErrorMessageVarArgsInternal(__VA_ARGS__);} while (0)
	...
	{
	public:
		TinyProfileZone(const char* name)
		{
			TinyEnterProfileZone(name);
		}

		~TinyProfileZone()
		{
			TinyLeaveProfileZone();
		}
	};
...
#define TinyStopProfile
	...
	///Don't use those internal functions directly, use the TinyPrintf or TinySetCustomPrintfFunc instead (or warning/error version)
