...
{
	// find_file searches files in 'data' folder in the current working directory and 
	// relative to executable directory and many of their parent directories.
	static bool find_file(const std::string& org_file_name, std::string& relative_file_name)
	{
		FILE* f = 0;
		...
		const std::string prefix[] = {"./", "./data/", "../data/", "../../data/", "../../../data/", "../../../../data/"};
		...
		for (int j = 0; j < 2; j++)
		{
			...
			if (l)
			{
				...
				int exe_name_pos = extract_path(exe_path, path_to_exe, TINY_MAX_EXE_PATH_LEN);
				if (exe_name_pos)
				{
					root = path_to_exe;
				}
	...
	static int extract_path(const char* file_name, char* path, int max_path_length)
	{
