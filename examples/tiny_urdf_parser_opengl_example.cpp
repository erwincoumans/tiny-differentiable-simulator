...
std::vector<double> initial_poses={0.98, 0.458, 0.31, -2.24, -0.30, 2.66, 2.32, 0.02, 0.02};//panda
                  ...
                  for (int s = 0; s < num_total_threads; s++)
                  {
                      for (int v = 0; v < num_base_instances; v++)
                      {
                          ...
                          if (visual_instance_id >= 0)
                          {
                              ...
                              OpenGLUrdfVisualizer<MyAlgebra>::TinyVisualLinkInfo& vis_link = visualizer.m_b2vis[uid1];

                              ::TINY::TinyVector3f pos(start_pos.x(),start_pos.y(),start_pos.z());
                              ::TINY::TinyQuaternionf orn(start_orn.x(),start_orn.y(),start_orn.z(),start_orn.w());
                              ...
                              MyAlgebra::Quaternion orn2d = MyAlgebra::quat_from_euler_rpy(vis_link.origin_rpy);
                              ::TINY::TinyQuaternionf orn2(orn2d.x(),orn2d.y(),orn2d.z(),orn2d.w());
                              ::TINY::TinyPosef global_tr (pos, orn);
                              ::TINY::TinyPosef local_tr (::TINY::TinyVector3f(visual_pos.x(),visual_pos.y(),visual_pos.z()), orn2);
                              ::TINY::TinyPosef full_tr =global_tr * local_tr;

                              full_tr.m_position[0] += sim_spacing * (s % square_id) - square_id * sim_spacing / 2;
                              full_tr.m_position[1] += sim_spacing * (s / square_id) - square_id * sim_spacing / 2;

                              visualizer.m_opengl_app.m_renderer->write_single_instance_transform_to_cpu(full_tr.m_position, full_tr.m_orientation, visual_instance_id);
                          }
                          instance_index++;
                      }
